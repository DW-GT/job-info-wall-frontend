...
export const ApplicationDetails = ({ applicationId }) => {
    ...
    const content = useSWR(
        process.env.API_ADRESS+`/api/application/getOffer/` + applicationId,
        (url: string) => axios(url).then((r) => r.data),
    ).data;
    ...
    return (
        <JobDetailsLayout>
            ...
            <IframeLayout>
                <iframe
                    src={process.env.API_ADRESS+content?.pdf_src + '#view=fit'}
                    width="100%"
                    height="100%"
                ></iframe>
                <StyledLink href={process.env.API_ADRESS+content?.pdf_src} target="_blank">
